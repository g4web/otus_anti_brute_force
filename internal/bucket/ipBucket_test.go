...
func TestIpBucket(t *testing.T) {
	t.Run("Test count limit", func(t *testing.T) {
		...
		isBanned, _ := ipBuckets.IPIsBanned("192.168.0.1")
		...
		isBanned, _ = ipBuckets.IPIsBanned("192.168.0.2")
		require.Equal(t, false, isBanned)
	})
	...
	t.Run("test whitelist", func(t *testing.T) {
		...
		isBanned, _ = ipBuckets.IPIsBanned("192.168.0.255")
		require.Equal(t, false, isBanned)

		isBanned, _ = ipBuckets.IPIsBanned("192.168.0.255")
		require.Equal(t, false, isBanned)

		isBanned, _ = ipBuckets.IPIsBanned("192.168.1.1")
		require.Equal(t, false, isBanned)
